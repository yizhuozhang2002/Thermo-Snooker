# -*- coding: utf-8 -*-
"""
Created on Sat Nov 19 19:38:52 2022
...
class Simulation:
    ...
    def __init__(self, pb=[[-1, 1], [1, 4]], R_c=10, R_b=[1, 1], v=[[1, 3], [-3, 2]], Number_of_balls=2):
        self.vb = []
        self._ball = []
        self.R_c = R_c
        ...
        for i in range(Number_of_balls):
            ...
            self._ball.append(Ball(p=pb[i], v=self.vb[i], R=1, m=1))
    ...
    def next_collision(self):
        ...
        for i in range(self.Number_of_balls):
            ...
            tb.append(self._ball[i].time_to_collision(
                self._ball[self.Number_of_balls-i-1]))
        tf = np.min(tb)
        tfc = np.min(tc)
        print("%s=%s" % ('tc=', tc))
        print("%s=%s" % ('tb=', tb))
        ...
        if tfc <= tf:
            ...
            self._ball[a].collide(self._ball[self.Number_of_balls-a-1])
